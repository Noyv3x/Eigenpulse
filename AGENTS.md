...
- **`mold` linker is required on Linux** — `.cargo/config.toml` passes `-fuse-ld=mold` for `aarch64-unknown-linux-gnu` and `x86_64-unknown-linux-gnu`. Without it the dev/test build fails at link time with `mold: command not found`. Install via `apt install mold` (Debian bookworm has 1.10.x, sufficient). The Dockerfile installs it in the chef stage; CI installs it per-job. macOS / Windows targets don't match those `[target.*]` entries, so mold is not required there. Reason for pinning: the multi-crate workspace links to ~190 crates; BFD `ld` peaks above 8 GB RSS during a release link, mold drops it to ~600 MB and runs ~5x faster.
