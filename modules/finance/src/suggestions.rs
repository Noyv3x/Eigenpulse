...
pub fn compute_suggestions(d: &LedgerData) -> Vec<Suggestion> {
    ...
    if let Some(worst) = d.budgets.iter()
        .filter(|b| b.amount > 0.0 && b.used / b.amount > BUDGET_OVERAGE_THRESHOLD)
        .max_by(|a, b| (a.used / a.amount).partial_cmp(&(b.used / b.amount)).unwrap_or(std::cmp::Ordering::Equal))
    {
        let pct = (worst.used / worst.amount * 100.0).round() as u32;
        let cat_name = d.categories.iter()
        ...
        out.push(Suggestion {
            ...
            title: format!("{cat_name} 已用 {pct}%"),
            meta: format!("剩 ¥{} · 距月底剩余天数请节制", ep_core::fmt_int(remaining)),
    ...
    if let Some(orphan) = d.txns.iter()
        .find(|t| t.linked_doc_id.is_none()
                  && (t.category_code == "HLT" || t.category_code == "EDU")
                  && Tag::parse(&t.tag) == Some(Tag::Exp))
    {
        let target_module = if orphan.category_code == "HLT" { "Fitness" } else { "Learning" };
        out.push(Suggestion {
            icon: IconKind::Link,
            title: format!("{} · 可关联 {}", orphan.merchant, target_module),
            meta: format!("{} 类别支出未关联到 {} 模块单号", orphan.category_code, target_module),
            link: Some("/finance".into()),
        });
    }
    ...
    if let Some(savings) = d.accounts.iter()
        .filter(|a| a.r#type == "Savings" && a.balance > SURPLUS_BALANCE_FLOOR)
        .max_by(|a, b| a.balance.partial_cmp(&b.balance).unwrap_or(std::cmp::Ordering::Equal))
    {
        let dca_amount = (savings.balance * SURPLUS_DCA_RATIO).round();
        out.push(Suggestion {
            icon: IconKind::Coin,
            title: format!("可定投 ¥{}", ep_core::fmt_int(dca_amount)),
            meta: format!("{} 余额 ¥{} · 建议 30% 分批转 ETF",
                          savings.name, ep_core::fmt_int(savings.balance)),
            link: Some("/finance".into()),
        });
    }
...
mod tests {
    ...
    fn fixture_data() -> LedgerData {
        LedgerData {
            ...
            txns: vec![
                Txn {
                    doc_id: "FIN-X".into(), occurred_at: 0,
                    merchant: "Keep Gym".into(), category_code: "HLT".into(),
                    account_code: "ACC-01".into(), amount: -298.0,
                    tag: "exp".into(), note: None, linked_doc_id: None,
                },
            ],
    ...
    fn budget_rule_picks_worst_overage() {
        ...
        let budget = s.iter().find(|s| s.icon == IconKind::Sparkle).expect("expected budget rule hit");
        assert!(budget.title.contains("餐饮"), "title: {}", budget.title);
        // 2900 / 3200 ≈ 90.625% → rounds to 91. Just check it's > 80 and ≤ 100.
        assert!(budget.title.contains("91%") || budget.title.contains("90%"),
                "expected 91% (or 90% on different round mode), got: {}", budget.title);
    }

    #[test]
    fn orphan_link_rule_finds_unlinked_hlt_expense() {
        let s = compute_suggestions(&fixture_data());
        let link = s.iter().find(|s| s.icon == IconKind::Link).expect("expected link rule hit");
    ...
    fn surplus_rule_fires_above_floor() {
        let s = compute_suggestions(&fixture_data());
        let coin = s.iter().find(|s| s.icon == IconKind::Coin).expect("expected surplus rule hit");
        // 22_800 × 0.30 = 6_840
