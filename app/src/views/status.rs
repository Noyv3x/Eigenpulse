...
pub fn StatusView() -> impl IntoView {
    ...
    view! {
        <div class="view">
            ...
            <div class="grid-2">
                <Card title=t(locale, "app.status.system_card.title")>
                ...
                <Card title=t(locale, "app.status.backup_card.title")
                    ...
                    <div style="margin-top:14px">
                        <ActionForm action=backup attr:class="hstack" attr:style="gap:10px;align-items:center;flex-wrap:wrap">
                            <button class="btn primary" type="submit"
                                    disabled=move || backup.pending().get()
                                    aria-busy=move || backup.pending().get().to_string()>
                            ...
                            // <ActionForm> button during hydration.
                        ...
                        <span class="new-token-slot">
                            {move || status.get().and_then(Result::ok).filter(|item| item.last_backup_exists).map(|_| view! {
                                <a class="btn" style="margin-top:10px" href="/api/_internal/admin/backup/latest" download="eigenpulse.epbackup">
