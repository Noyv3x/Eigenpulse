...
pub struct SmtpNotifier { cfg: SmtpConfig }
...
impl Notifier for SmtpNotifier {
    fn kind(&self) -> &'static str { "smtp" }
    async fn send(&self, msg: &NotifyMessage) -> anyhow::Result<()> {
        let subject = format!("[Eigenpulse · {}] {}", msg.severity.as_str().to_uppercase(), msg.title);
        let mut html = format!(
            "<h2 style=\"font-family:sans-serif\">{}</h2>",
            html_escape(&msg.title)
        );
        if let Some(b) = &msg.body {
            html.push_str(&format!("<p style=\"font-family:sans-serif;color:#4b5563\">{}</p>", html_escape(b)));
        }
        if let Some(d) = &msg.doc_ref {
            html.push_str(&format!("<p style=\"font-family:monospace;color:#9ca3af\">ref · {}</p>", html_escape(d)));
        }
