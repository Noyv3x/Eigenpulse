...
fn normalize_channel_fields(
    ...
    validate_notifier_config(&kind, &config_json)?;
    Ok(ChannelInput {
        kind,
        name,
        config_json,
...
/// Validate a provider configuration without exposing provider implementations
/// or the internal notifier trait across the crate boundary.
pub fn validate_notifier_config(kind: &str, config_json: &str) -> anyhow::Result<()> {
    crate::bus::build_notifier(kind, config_json).map(drop)
}

pub async fn test_channel(kind: &str, config_json: &str) -> anyhow::Result<()> {
    let n = crate::bus::build_notifier(kind, config_json)?;
