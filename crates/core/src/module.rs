use crate::{AppState, ModuleDescriptor};
...
macro_rules! assert_module_migrations_registered {
    ($module:expr) => {{
        $crate::assert_module_migrations_registered!($module, "migrations")
    }};
    ($module:expr, $directory:literal) => {{
        let manifest_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let migration_dir = manifest_dir.join($directory);
