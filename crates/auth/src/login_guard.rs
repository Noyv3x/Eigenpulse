...
///
/// This is a *fixed* window. Expired buckets are evicted lazily and the map has
/// a hard capacity; addresses arriving after that capacity share an overflow
/// bucket. An attacker therefore cannot grow process memory with forged IPs.
///
...
impl LoginThrottle {
    ...
    pub fn new(max_attempts: u32, window: Duration) -> Self {
        Self::new_bounded(max_attempts, window, DEFAULT_MAX_ENTRIES)
    }

    /// Build a limiter with an explicit bucket cap. At least one slot is
    /// reserved for the overflow bucket.
    pub fn new_bounded(max_attempts: u32, window: Duration, max_entries: usize) -> Self {
        Self {
            max_attempts,
            window,
    ...
    async fn check_and_record_at(&self, key: &str, now: Instant) -> Result<(), RetryAfter> {
        ...
        if !map.contains_key(key) && map.len() >= self.max_entries.saturating_sub(1) {
            map.retain(|_, entry| now.duration_since(entry.start) < self.window);
        }
        let bucket = if map.contains_key(key) || map.len() < self.max_entries.saturating_sub(1) {
            key
        } else {
            OVERFLOW_KEY
        };
        let entry = map.entry(bucket.to_string()).or_insert(Window {
            attempts: 0,
            start: now,
        });
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn throttle_caps_unique_keys_and_uses_overflow_bucket() {
        let throttle = LoginThrottle::new_bounded(2, Duration::from_secs(60), 4);
        let t0 = Instant::now();
        for key in ["a", "b", "c", "d", "e", "f"] {
            let _ = throttle.check_and_record_at(key, t0).await;
        }
        assert_eq!(throttle.entry_count().await, 4);
        // d/e/f share the overflow bucket, which is exhausted after two
        // attempts rather than allocating an unbounded number of entries.
        assert!(throttle.check_and_record_at("new-key", t0).await.is_err());
    }

    #[tokio::test(flavor = "current_thread")]
    async fn expired_entries_are_evicted_before_overflow_is_used() {
        let window = Duration::from_secs(10);
        let throttle = LoginThrottle::new_bounded(1, window, 3);
        let t0 = Instant::now();
        assert!(throttle.check_and_record_at("old-a", t0).await.is_ok());
        assert!(throttle.check_and_record_at("old-b", t0).await.is_ok());
        let later = t0 + window + Duration::from_secs(1);
        assert!(throttle.check_and_record_at("fresh", later).await.is_ok());
