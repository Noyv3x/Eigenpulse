...
/// Read-only validity check for long-lived transports such as SSE. Unlike
/// [`lookup_session`], this deliberately does not refresh `last_seen`, extend
/// expiry, or delete an expired row: an open stream must not keep a stolen or
/// otherwise idle cookie alive forever.
pub async fn session_is_valid(pool: &SqlitePool, token: &str) -> anyhow::Result<bool> {
...
mod tests {
    ...
    /// don't run the full platform baseline because it has FK references
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn read_only_validity_detects_revocation_without_renewing() {
        let pool = fixture_pool().await;
        let now = ep_core::unix_now();
        sqlx::query("INSERT INTO session VALUES ('stream', 1, 0, ?1, 123)")
            .bind(now + SESSION_LIFETIME_SECS)
            .execute(&pool)
            .await
            .unwrap();

        assert!(session_is_valid(&pool, "stream").await.unwrap());
        let last_seen: i64 =
            sqlx::query_scalar("SELECT last_seen FROM session WHERE token = 'stream'")
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(last_seen, 123);

        logout_destroy_session(&pool, "stream").await.unwrap();
        assert!(!session_is_valid(&pool, "stream").await.unwrap());
    }
