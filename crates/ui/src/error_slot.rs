...
/// load-bearing for tachys hydration because a moving placeholder neighbour
/// can panic the text-node walker. When the bound action last
...
{
    view! {
        <span class="error-slot" style=style aria-live="polite">
            {move || {
                action
                    .value()
                    .get()
                    .and_then(|r| r.err())
                    .map(|e| view! { <span class="tag rose" role="alert">{ep_i18n::server_fn_error_text(&e)}</span> })
            }}
        </span>
    }
}
