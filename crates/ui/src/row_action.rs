...
/// `ActionForm` that drives the underlying server action. The action form stays
/// owned by this component while the shared native [`crate::Dialog`] supplies
/// modal semantics and focus management.
///
/// `value` ships as the hidden input's value. The default field is the
/// module-local integer `id`; callers may override it for another action input.
#[component]
...
{
    let field = field.unwrap_or_else(|| "id".into());
