...
pub enum Theme {
    Light,
    // Dark is the hero / default presentation; matches the `dark:comfortable`
    // fallback in assets/theme-init.js so SSR, the anti-FOUC pre-paint attrs,
    // and the post-mount restore all agree for a first-time visitor.
    #[default]
    Dark,
}
impl Theme {
...
impl TweakState {
    ...
    pub fn parse_short(s: &str) -> Self {
        ...
        let theme = Theme::parse(parts.next().unwrap_or("dark"));
