...
│   ├── fitness/            # schema + 训练记录 CRUD + 趋势/连续训练
│   ├── learning/           # schema + 书籍/笔记 CRUD + 课程进度视图
│   └── mod_marketplace/    # 已注册模块总览
...
3. 按 scope 控制权限：`activity:read` / `fin:read` / `fin:write` / `notify:write` / `*`。
...
- Service Worker 对 `/pkg/*` 使用 network-first，对普通 `/static/*` 使用 stale-while-revalidate；HTML/navigation、API、SSE 与 `/static/theme-init.js` 始终走网络，避免登出、会话或首屏主题脚本变化后展示陈旧内容
