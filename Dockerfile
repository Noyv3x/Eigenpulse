...
# BuildKit resolves an unqualified FROM to TARGETPLATFORM. The SSR binary is
# therefore compiled by the target-architecture Rust toolchain (under QEMU for
# arm64 on an amd64 runner) instead of leaking a BUILDPLATFORM binary into the
# target runtime image. The hydrate bundle remains architecture-independent.
...
RUN cargo chef cook --release --package eigenpulse --features ssr --no-default-features --recipe-path recipe.json
RUN cargo chef cook --release --package eigenpulse --features hydrate --no-default-features \
    --target wasm32-unknown-unknown --recipe-path recipe.json
COPY . .
# cargo-leptos emits the canonical `/pkg/<name>.wasm` hydration artifact;
# the runtime serves package files without compatibility aliases.
...
COPY --from=builder /app/target/site/pkg /app/site/pkg
...
ENV LEPTOS_OUTPUT_NAME=eigenpulse \
    ...
    EP_MODULE_DATA_ROOT=/data/modules \
    RUST_LOG=info
VOLUME ["/data"]
EXPOSE 3000
